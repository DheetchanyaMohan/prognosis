...
def _data_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    """metrics_tool now resolves config.yaml's location via
    run_paths.build_run_artifact_paths, which depends on
    settings.data_root — point it at tmp_path for the duration of each
    test, matching where _seed_run below actually writes files."""
...
def _seed_run(db: Session, run_id: str, dropout: float = 0.3, lr: float = 0.001) -> None:
    ...
    _write_config(run_dir / "config.yaml", run_id, dropout=dropout, lr=lr)
    ...
    persist_run(
            db=db, experiment=experiment, run_id=run_id, status="complete", epoch_history=history
        )

# --- analyze_run ---------------------------------------------------------


def test_analyze_run_matches_direct_analysis_layer_call(db_session: Session) -> None:
...
def test_summarize_run_includes_config_and_diagnostics(db_session: Session) -> None:
    _seed_run(db_session, "run_001", dropout=0.5, lr=0.01)

    view = summarize_run("run_001", db=db_session)

    assert view.run_id == "run_001"
    assert view.experiment_name == EXPERIMENT_NAME
...
def test_compare_runs_detects_differing_hyperparameters(db_session: Session) -> None:
    _seed_run(db_session, "run_101", dropout=0.0, lr=0.001)
    _seed_run(db_session, "run_102", dropout=0.5, lr=0.001)
...
def test_compare_runs_includes_diagnostics_for_both(db_session: Session) -> None:
...
def test_compare_runs_raises_when_either_missing(db_session: Session) -> None:
    _seed_run(db_session, "run_101")
    with pytest.raises(RunNotFoundError):
